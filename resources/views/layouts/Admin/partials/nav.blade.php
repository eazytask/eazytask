    ...
    <div class="navbar-container d-flex content">
        <div class="bookmark-wrapper d-flex align-items-center">
            ...
            <ul class="nav navbar-nav">
                <li>Company: <span class="user-name font-weight-bolder text-uppercase">{{Auth::user()->company_roles->first()->company->company_code}}</span></li>
            </ul>
        </div>
        <ul class="nav navbar-nav align-items-center ml-auto">
            ...
            <li class="nav-item dropdown dropdown-notification mr-25"><a class="nav-link" onclick="markAsRead()" href="javascript:void(0);" data-toggle="dropdown"><i class="ficon" data-feather="bell"></i><span class="badge badge-pill badge-danger badge-up" id="notify_count">{{Auth::user()->unreadNotifications->count()}}</span></a>
                <ul class="dropdown-menu dropdown-menu-media dropdown-menu-right">
                    ...
                    <li class="scrollable-container media-list" style="height:25rem !important">
                        <div class="{{$notifications->count()?'d-block':'d-none'}}" id="notifications" >
                            ...
                            <a class="d-flex {{bg($notification->data['status']??'')}}" href="{{$type[$notification->type]??'javascript:void(0);'}}">
                                <div class="media d-flex align-items-start {{$notification->data['status']=='warning'?'bg-light-warning':''}}">
                                    <div class="media-left">
                                        ...
                                        @endphp
                                        <div class="avatar"><img src="{{ 'https://api.eazytask.au/'.$n_image}}" alt="avatar" width="32" height="32"></div>
                                    </div>
                                    <div class="media-body">
                                        <p class="media-heading font-small-3"><span class="{{$notification->read_at?'':'font-weight-bolder'}}">
                                                {{$notification->data['msg']}}
                                        </p>
                                    </div>
                                    <div class="media-right">
                                        <label class="">{{\Carbon\Carbon::parse($notification->created_at)->diffForHumans()}}</label>
                                    </div>
                                </div>
                            </a>
                            @endforeach
                        ...
                        <a class="{{$notifications->count()?'d-none':'d-flex'}}" href="javascript:void(0)" id="no_notification">
                            <p class="p-2 m-auto" style="margin-top: 10rem !important;">No notifications</p>
                        </a>
                    </li>
                    <li class="dropdown-menu-footer"><a class="btn btn-primary btn-block" href="javascript:void(0)" onclick="deleteAllNotifications()">Clear all notifications</a></li>

                </ul>
            </li>
            <li class="nav-item dropdown dropdown-user">
                <a class="nav-link dropdown-toggle dropdown-user-link" id="dropdown-user" href="javascript:void(0);" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                    <div class="user-nav d-sm-flex d-none"><span class="user-name font-weight-bolder">{{ Auth::user()->name }} {{ Auth::user()->mname }}
                    ...
                    <span class="avatar"><img class="round" src="{{ 'https://api.eazytask.au/'.Auth::user()->image }}" alt="avatar" height="40" width="40"><span class="avatar-status-online"></span></span>
                </a>
                <div class="dropdown-menu dropdown-menu-right" aria-labelledby="dropdown-user">
                    ...
                    <a class="dropdown-item" href="/home/time/off"><i class="mr-50" data-feather="clock"></i> Time off</a>
                    @endif
